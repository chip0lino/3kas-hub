      ...
      jsonData.forEach((row, number) => {
        if (number >= (pageNumber - 1) * itemsOnPage && number < pageNumber * itemsOnPage){
          ...
          if (window.location.pathname.includes('adminpanel.html')) {
            ...
            const deleteButtonWrapper = document.createElement('span');

            deleteButtonWrapper.style.float = 'right'; // make the span float to the right
            deleteButtonWrapper.style.marginLeft = '10px'; // add 10px margin to the left\
            ...
            deleteButton.onclick = function() {
              ...
              console.log('ID сотрудника:', id); // вывести ID сотрудника в консоль
              if (!isNaN(id)) {
                const employeeIndex = filteredData.findIndex(employee => employee["ID"] === parseInt(id));
                if (employeeIndex !== -1) {
                  deleteEmployee(id); // вызвать функцию deleteEmployee с ID
                } else {
                  console.error('Сотрудник не найден');
                }
              } else {
                console.error('ID сотрудника не является числом');
              }
            };
